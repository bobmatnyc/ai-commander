//! Auggie (Augment Code) runtime adapter.
...
/// Adapter for Auggie (Augment Code) CLI.
pub struct AuggieAdapter {
    info: AdapterInfo,
}

impl AuggieAdapter {
    /// Creates a new Auggie adapter.
    pub fn new() -> Self {
        Self {
            info: AdapterInfo {
                id: "auggie".to_string(),
                name: "Auggie".to_string(),
                description: "Augment Code AI coding assistant".to_string(),
...
impl RuntimeAdapter for AuggieAdapter {
    ...
    fn error_patterns(&self) -> &[&str] {
        &[r"(?i)^error:", r"(?i)failed", r"(?i)^error\s"]
    }
}
...
mod tests {
    ...
    fn test_adapter_info() {
        let adapter = AuggieAdapter::new();
        assert_eq!(adapter.info().id, "auggie");
        assert_eq!(adapter.info().name, "Auggie");
        assert_eq!(adapter.info().command, "auggie");
        assert!(adapter.info().default_args.is_empty());
    }
